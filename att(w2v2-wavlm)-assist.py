...
class Module(nn.Module):
    def __init__(self, embed_dim=128, depth=4, device="cuda", bidirectional=True):
        ...
        self.first_bn = nn.BatchNorm2d(num_features=1).to(device)
        self.first_bn1 = nn.BatchNorm2d(num_features=64).to(device)
        self.drop = nn.Dropout(0.5, inplace=True).to(device)
        self.drop_way = nn.Dropout(0.2, inplace=True).to(device)
        self.selu = nn.SELU(inplace=True).to(device)
    ...
    def forward(self, inputs, inputs2=None, Freq_aug=None):
        ...
        x = torch.cat([x1, x2], dim=2).to(self.device)

        # 新生成的张量显式指定设备
        x = F.interpolate(x.permute(0, 2, 1), size=201, mode='linear', align_corners=True).permute(0, 2, 1)
        x = x.to(self.device)

        # 添加通道维度并处理
        x = x.transpose(1, 2).unsqueeze(dim=1).to(self.device)
        x = F.max_pool2d(x, (3, 3))
        x = self.first_bn(x)  # BatchNorm 操作，确保输入和权重都在同一设备
        x = self.selu(x)
