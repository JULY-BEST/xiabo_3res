...
class Module(nn.Module):
    def __init__(self, embed_dim=128, depth=4, device="cuda", bidirectional=True):
        ...
        # 配置交叉多头注意力
        self.cross_attention = CrossMultiheadAttention(embed_dim=embed_dim, num_heads=8)
        ...
        self.self_attention = EncoderSelfAttention(
            input_dim=embed_dim, d_model=d_model,
            d_k=d_k, d_v=d_v, n_head=n_head,
            n_module=n_module, dropout_transformer=dropout_transformer
        ).to(self.device)

        # 通道增强机制 SE Block
        self.se_block = SEBlock(embed_dim * 2).to(self.device)

        # 全连接层
        self.fc = nn.Linear(embed_dim * 2, 2).to(self.device)  # 二分类输出

    def forward(self, inputs, inputs2=None, Freq_aug=None):
        x = inputs
        
        # 提取多视角特征
        text_view = self.text_view_extract(x)  # 形状: (batch_size, seq_len, embed_dim)
        wav_view = self.wav_view_extract(x)  # 形状: (batch_size, seq_len, embed_dim)

        # 交叉多头注意力
        out1, attn1 = self.cross_attention(text_view, wav_view, wav_view)
        out2, attn2 = self.cross_attention(wav_view, text_view, text_view)

        # 融合特征
        x1 = out2
        x2 = out1
        x = torch.cat([x1, x2], dim=2)  # (batch_size, seq_len, embed_dim * 2)

        # 通道增强
        x = x.permute(0, 2, 1)  # 转换为 (batch_size, embed_dim * 2, seq_len)
        x = self.se_block(x)  # SE Block 增强
        x = x.permute(0, 2, 1)  # 转回 (batch_size, seq_len, embed_dim * 2)

        # 自注意力模块
        x = self.self_attention(x)  # 输出形状: (batch_size, seq_len, embed_dim * 2)

        # 获取最后一个时间步的特征
        x = x[:, -1, :]  # (batch_size, embed_dim * 2)
