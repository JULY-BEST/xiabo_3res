...
class Module(nn.Module):
    def __init__(self, embed_dim=128, depth=4, device="cuda", bidirectional=True):
        ...
        # 如果LSTM是双向的，嵌入维度会加倍
        self.fc_input_dim = embed_dim * 2 if bidirectional else embed_dim

        # 全连接层
        self.fc = nn.Linear(self.fc_input_dim, 2).to(self.device)  # 确保全连接层也在正确的设备上
    ...
    def forward(self, inputs, inputs2=None, Freq_aug=None):
        ...
        # 输入到三层LSTM
        x, _ = self.lstm(x)  # 输出为 (batch_size, seq_len, hidden_dim * 2) if bidirectional=True

        # 取最后一个时间步的特征
        x = x[:, -1, :]  # (bs, hidden_dim * 2) if bidirectional=True
