...
class DynamicConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, reduction=4):
        ...
        self.fc2 = nn.Linear(in_channels // reduction, out_channels * kernel_size)
    ...
    def forward(self, x):
        ...
        b, c, l = x.size()
        weights = self.global_pool(x).view(b, c)  # Global pooling
        weights = F.relu(self.fc1(weights))  # Reduction layer
        weights = self.fc2(weights).view(b, self.out_channels, self.kernel_size)  # Dynamic kernel
        weights = F.softmax(weights, dim=-1)  # Normalize kernel weights
        
        # Convolve with dynamic kernel
        x = F.unfold(x.unsqueeze(-1), (self.kernel_size, 1), padding=(self.kernel_size // 2, 0))
        x = x.view(b, c, self.kernel_size, l).permute(0, 3, 1, 2)  # (b, l, c, kernel_size)
        x = torch.einsum('blci,boi->blco', x, weights)  # Apply dynamic weights
        return x.squeeze(-1).permute(0, 2, 1)  # Return to (b, c, l)

class MultiHeadAttention_(nn.Module):
    """
    Multi-head attention layer with advanced token mixing via dynamic convolutions.
    """
    def __init__(self, d_model, d_k, d_v, h, kernel_sizes=(3, 5, 7), reduction=4, dropout=0.1):
        super(MultiHeadAttention_, self).__init__()

        # Multi-scale dynamic convolution
        self.token_mixers = nn.ModuleList([
            DynamicConv(d_model, d_model, kernel_size, reduction=reduction) for kernel_size in kernel_sizes
        ])
        self.dropout = nn.Dropout(p=dropout)
        self.layer_norm = nn.LayerNorm(d_model)
    ...
    def forward(self, x):
        ...
        att_outputs = [token_mixer(x) for token_mixer in self.token_mixers]
        att = sum(att_outputs) / len(att_outputs)  # Aggregate multi-scale outputs
        ...
        return self.layer_norm(x.squeeze(1) + att)
