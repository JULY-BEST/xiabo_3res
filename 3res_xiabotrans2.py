...
class WaveletAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        Returns:
            x_out: [batch_size, seq_len, embed_dim]
        """
        batch_size, seq_len, embed_dim = x.size()
        assert embed_dim == self.embed_dim, "Embedding dimension mismatch"

        # 小波变换
        x_dwt, _ = self.dwt(x)  # 小波变换输出 (低频，高频)
        x_dwt = x_dwt.squeeze(1)  # [batch_size, seq_len, embed_dim]

        # 计算 Query, Key, Value
        Q = self.query(x_dwt)  # [batch_size, seq_len, embed_dim]
        K = self.key(x_dwt)  # [batch_size, seq_len, embed_dim]
        V = self.value(x_dwt)  # [batch_size, seq_len, embed_dim]

        # 计算注意力权重
        attention_scores = torch.bmm(Q, K.transpose(1, 2))  # [batch_size, seq_len, seq_len]
        attention_weights = self.softmax(attention_scores / (self.embed_dim ** 0.5))  # Scaled Dot-Product Attention

        # 应用注意力权重到 Value
        attended_features = torch.bmm(attention_weights, V)  # [batch_size, seq_len, embed_dim]

        # 小波逆变换
        x_reconstructed = self.idwt(attended_features.unsqueeze(1))  # 恢复特征 [batch_size, seq_len, embed_dim]

        # 投影回原始维度
        x_out = self.proj(x_reconstructed.squeeze(1))  # [batch_size, seq_len, embed_dim]
