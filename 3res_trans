...
class Module(nn.Module):
    def __init__(self, embed_dim=128, depth=4, device="cuda", bidirectional=True):
        ...
        self.residual_block4 = Residual_block(embed_dim).to(self.device)
        self.residual_block5 = Residual_block(embed_dim).to(self.device)
        self.residual_block6 = Residual_block(embed_dim).to(self.device)

        # Transformer: 使用 nn.TransformerEncoder 和 nn.TransformerEncoderLayer
        encoder_layers = nn.TransformerEncoderLayer(d_model=embed_dim, nhead=8, dropout=0.1)
        self.transformer_encoder = nn.TransformerEncoder(encoder_layers, num_layers=4).to(self.device)
    ...
    def forward(self, inputs, inputs2=None, Freq_aug=None):
        ...
        # 转换维度为 [sequence_length, batch_size, embed_dim] 以便输入到 Transformer 中
        x = x.transpose(0, 1)  # [50, 1, 128]

        # 输入到 Transformer
        x = self.transformer_encoder(x)  # [50, 1, 128]

        # 转换回原来的维度 [batch_size, sequence_length, embed_dim]
        x = x.transpose(0, 1)  # [1, 50, 128]
